...
for img_no in range(1, 30):
    for side in ['left', 'right']:
        ...
        image = cv2.imread('../data/calib_imgs/rotate/{}.jpg'.format(filename))
        ...
        if ret == True:
            ...
            imgpoints_dict[filename] = corners.reshape(-1, 2)
            # print(corners[0][0])
            # # # 将角点在图像上显示
...
iop = {
    ...
    'tvecs': tvecs,
    'objpoints': objp,
    'imgpoints': imgpoints_dict
}

# 儲存相機參數以及在每張影像的匹配點(imgpoints)
tools.save(iop, '../data/calib_imgs/rotate/iop.pk')